...
namespace Client
{
    public class MessageQueueClient
    {
        ...
        public bool initialize(string address, ushort port)
        {
            ...
            m_messageCommand[Shared.Messages.Type.GameSize] = () => { return new GameSize(); };
            m_messageCommand[Shared.Messages.Type.Turn] = () => { return new Turn(); };
                ...
                while (m_keepRunning)
                {
                    try
                    {
                        ...
                        if (bytesReceived > 0)
                        {
                            ...
                            // added this try catch to catch any errors and gracefully handle them rather than crashing
                            try
                            {
                                // Attempt to retrieve the message command from the dictionary
                            ...
                            {
                                // Handle the case where the message type is not recognized
                                Console.WriteLine("Error: Unknown message type received. Details: " + ex.Message);
                                // You may want to log the error or take other appropriate actions here
                            }
                            catch (Exception ex)
                            {
